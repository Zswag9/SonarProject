...
class player:
    ...
    def enterPlayerMove(whichPlayer):
        ...
        while True:
            ...
            move = move.split()
            if len(move) == 2 and move[0].isdigit() and move[1].isdigit() and player.isValidMove(int(move[0]), int(move[1])):
...
while True:
    ...
    while sonarDevices > 0:
        ...
        if P1.sonar > 1 and P2.sonar > 1: extraSsonar = 's'
        else: extraSsonar = ''
        if len(theChests) > 1: extraSchest = 's'
        else: extraSchest = ''
        if p%2 == 1:
            print('You have %s sonar device%s left. %s treasure chest%s remaining.' % (P1.sonar, extraSsonar, len(theChests), extraSchest))
        else:
            print('You have %s sonar device%s left. %s treasure chest%s remaining.' % (P2.sonar, extraSsonar, len(theChests), extraSchest))
